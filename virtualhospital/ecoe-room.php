...
<body style="min-height: 100vh;margin: 0;">  
    ...
    $reader->open("xml/" . $room . ".xml");
    while ($reader->read()) {
        if ($reader->nodeType == XMLReader::ELEMENT && $reader->name == "object") {
            echo "<p>Name: " . $reader->getAttribute("name") . "</p>";
            echo "<hr/>";
        }
    }
    $reader->close();
    ...
    <script type="importmap">
            {
                "imports": {
                    "three": "https://threejs.org/examples/../build/three.module.js",
                    "three/addons/": "https://threejs.org/examples/jsm/"
                }
            }
        </script>
    <script>
            let xmlfile = <?php echo json_encode($room); ?>;
            console.log(xmlfile);
    </script>
